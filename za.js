 ...
 * ├── package.json
 * └── database/
 *     ├── owner.json         ← ["628xxx"]  (owner tambahan, opsional)
 *     ├── premium.json       ← ["628xxx@s.whatsapp.net"]
 *     ├── registered.json    ← []
 *     ├── antitoxic.json     ← ["kata kasar", ...]
 *     ├── antivirus.json     ← ["628xxx@g.us", ...]
 *     ├── bad.json           ← ["628xxx@g.us", ...]
 *     ├── bacotoff.json      ← ["628xxx@g.us", ...] (grup yg matiin mode bacot/nyinyir)
...
const CONFIG = {
    PREFIX: '.',
    BOT_NAME: 'rijal💫',
    BOT_NUMBER: '6285165639535',   // ← ganti dengan nomor bot kamu
...
let bacotOffGrp   = readDB('bacotoff.json');      // ["groupid@g.us"] -> grup yang mode bacot/nyinyir-nya OFF
...
// ──────────────────────────────────────────────
//  AUTO ADU BACOT (balasan random saat di-reply)
// ──────────────────────────────────────────────
// Dipicu saat seseorang mereply pesan BOT (bukan command), KHUSUS DI GRUP.
// Bot otomatis balas dengan satu baris bacotan/nyinyiran. Kalau pesan yang
// nge-reply itu ngandung kata kasar tertentu (anjg, goblok, dst), bot akan
// nyari balasan yang "nyambung" sama kata kasarnya, bukan asal random.
// Fitur ini bisa di on/off-in per grup pakai: .bacot on / .bacot off

// Balasan generik (dipakai kalau nggak ada kata kasar spesifik yang cocok)
const BACOT_REPLIES = [
    'Halah, gitu doang udah baper 😎',
    'Woi santai, ini bot bukan psikolog 😂',
    'Mau adu bacot? Bot ini nggak pernah kalah debat 🔥',
    'Capek mikir? Sama, capek liat chat kamu 💀',
    'Skill issue, bukan bot issue 😏',
    'Bacotan kamu kurang panas, coba lagi ☕',
    'Diem dong, baterai bot mau dicas 🔋',
    'Wah berani juga ngegas ke bot 🤡',
    'Kalah argumen sama AI? Wajar sih 😌',
    'Santuy, ini cuma bot, jangan dianggap serius 😆',
    'Reply lagi kalau berani, gas terus 😈',
    'Bot ini cuma jawab, kamu yang baper duluan 🙂',
    'Mantap, tapi belum cukup buat ngalahin bacotan bot 💅',
    'Ya udah, anggap aja kamu menang biar cepet 🏳️',
    'Niat ngegas tapi bacotannya kayak angin lewat doang 🍃',
    'Energi buat ngetik segini banyak, sayang kalau cuma buat baper 😮‍💨',
    'Bot doang yang ladenin, di dunia nyata mana ada yang sesabar bot 🙄',
    'Tarik napas dulu, reply lagi kalau udah siap kalah lagi 😌',
    'Receh banget bacotannya, upgrade dulu skill ngegasnya 📉',
    'Kalem aja, ini grup WA bukan ring tinju 🥊',
    'Udah panas duluan, bot masih santuy sambil ngopi ☕',
    'Lucu, ngegas ke AI tapi capeknya manusia 🤖➡️🧍',
    'Mau menang debat sama bot? Siapin mental dulu ya 💪',
    'Itu bacotan apa curhatan, kok jadi mellow gini 🥲',
    'Bot nggak punya perasaan, jadi bacotanmu nggak ngefek sama sekali 🤷',
];
const getBacot = () => getRandom(BACOT_REPLIES);

// Balasan khusus per kata kasar -> bot "nyinyirin balik" sesuai konteks kata
// yang dipakai, bukan asal jawab generik. Tiap kategori ada beberapa variasi
// biar nggak kelihatan template-an terus.
const INSULT_REPLIES = [
    {
        triggers: ['anjing', 'anjg', 'anj'],
        replies: [
            'Anjing? Kau malah lebih dari itu, kau ini celeng berkedok manusia 🐗',
            'Manggil bot anjing? Anjing aja masih setia sama tuannya, lah kau modal ngegas doang 🐶',
            'Anjing masih bisa jagain rumah, kau jagain emosi sendiri aja nggak bisa 😏',
            'Ngomong anjing ke bot, padahal yang lebih galak siapa coba kalau dipikir-pikir 🐕',
            'Anjing nggak pernah baper digituin, kau malah ngegas duluan. Siapa yang lebih layak dipanggil gitu ya 🤔',
        ],
    },
    {
        triggers: ['babi'],
        replies: [
            'Babi makan apa aja, kau makan omongan sendiri aja keberatan 🐷',
            'Manggil babi? Babi nggak ngetik kasar jam segini, kau yang kelewatan 🐖',
            'Babi cuma julukan receh, prestasimu yang masih nol itu fakta beneran 😌',
        ],
    },
    {
        triggers: ['kampret'],
        replies: [
            'Kampret terbang malam doang nyari makan, kau ngegas siang bolong nyari ribut 🦇',
            'Disangka kampret, padahal yang lebih berisik ya bacotanmu sendiri 😆',
        ],
    },
    {
        triggers: ['bangsat', 'bgst'],
        replies: [
            'Bangsat tuh julukan, tapi yang ngetik komen receh ya kau sendiri 😂',
            'Bgst? Singkatan "Banyak Gas, Sedikit Tindakan" kayaknya pas banget buat kau 🔥',
        ],
    },
    {
        triggers: ['monyet'],
        replies: [
            'Monyet pintar manjat pohon, kau cuma pintar manjat emosi doang 🐒',
            'Disebut monyet, tapi yang loncat-loncat baper duluan ya kau 😅',
        ],
    },
    {
        triggers: ['goblok', 'gblk', 'tolol', 'bego', 'bodoh'],
        replies: [
            'Manggil bot goblok? Bot ini diprogram buat jawab, kau yang ngetik tanpa dipikir dulu 🤖',
            'Tolol itu kalau ngegas ke bot terus capek sendiri, coba introspeksi bentar deh 😅',
            'Bego nggak bego, yang jelas waktumu abis cuma buat marah ke chat doang 🕐',
            'Soal bodoh, mendingan kita samaan aja biar nggak ada yang kalah 🤝',
        ],
    },
    {
        triggers: ['jancok', 'jancuk', 'dancok'],
        replies: [
            'Jancok keluar juga, mantap kosakatanya. Sayang tetep kalah adu bacot sama bot 😎',
            'Jancuk meluncur mulus, sayangnya nggak ngefek ke prosesor bot 💻',
        ],
    },
    {
        triggers: ['tai', 'taik'],
        replies: [
            'Disebut tai? Tai aja masih berguna buat pupuk, kau buat apa coba 🌱',
            'Daripada ngomong tai, mending nabung energi positif, biar harimu nggak runyam 🍀',
        ],
    },
    {
        triggers: ['keparat', 'sialan'],
        replies: [
            'Sialan-sialan tapi tetep balik nge-reply bot, berarti baper juga kan 😏',
            'Keparat tuh kata berat, mending nabung sabar daripada nabung emosi 🙏',
        ],
    },
];

// Cari balasan yang nyambung sama kata kasar di pesan; kalau nggak ada yang
// cocok, fallback ke balasan generik biasa.
function getBacotReply(teksAsli) {
    const lower = (teksAsli || '').toLowerCase();
    for (const grup of INSULT_REPLIES) {
        if (grup.triggers.some(kata => lower.includes(kata))) return getRandom(grup.replies);
    }
    return getBacot();
}

// Mode bacot/nyinyir per grup: default AKTIF, kecuali grup itu ada di daftar
// bacotOffGrp (sudah di-.bacot off-kan).
const isBacotOn = (jid) => !bacotOffGrp.includes(jid);
...
async function startBot() {
    ...
    //  HELPER SEND
    // ──────────────────────────────────────────
    // sentMsgIds: nyimpen ID pesan yang BENERAN dikirim sendiri oleh bot.
    // Dipakai buat ngecek "ini reply ke pesan bot apa bukan" dengan akurat.
    // Ngecek lewat JID participant doang nggak selalu bisa diandalkan,
    // karena WhatsApp kadang ngirim field participant dalam bentuk @lid
    // yang nggak match sama JID asli bot walau itu beneran reply ke pesan
    // bot (ini bug/perilaku resmi WA yang lagi rolling out, bukan salah
    // logic kita). Ngecek pakai stanzaId/messageId jauh lebih akurat
    // karena ID itu kita sendiri yang generate.
    const sentMsgIds = new Set();
    const MAX_TRACKED_IDS = 500;
    function trackSent(sentMsg) {
        const id = sentMsg?.key?.id;
        if (id) {
            sentMsgIds.add(id);
            if (sentMsgIds.size > MAX_TRACKED_IDS) sentMsgIds.delete(sentMsgIds.values().next().value);
        }
        return sentMsg;
    }
    const sendTracked = async (jid, content, opts) => trackSent(await sock.sendMessage(jid, content, opts));

    const reply = async (jid, text, quoted) => trackSent(await sock.sendMessage(jid, { text }, { quoted }));
    const sendImg = async (jid, buffer, caption = '', quoted) => trackSent(await sock.sendMessage(jid, { image: buffer, caption }, { quoted }));
    const sendAudio = async (jid, buffer, ptt = false, quoted) => trackSent(await sock.sendMessage(jid, { audio: buffer, ptt, mimetype: 'audio/mp4' }, { quoted }));
    const sendVideo = async (jid, buffer, caption = '', quoted) => trackSent(await sock.sendMessage(jid, { video: buffer, caption }, { quoted }));
    const sendDoc = async (jid, buffer, filename, mimetype, quoted) => trackSent(await sock.sendMessage(jid, { document: buffer, fileName: filename, mimetype }, { quoted }));
...
Dikenali sebagai owner? ${isOwner(senderInfo) ? '✅ YA' : '❌ TIDAK'}`;
                ...
                // ═══════════════════════════════════════
                //  GRUP SETTINGS: MODE BACOT/NYINYIR
                // ═══════════════════════════════════════
                else if (cmd === 'bacot') {
                    if (!isGrp) return reply(jid, '❌ Fitur bacot/nyinyir cuma bisa diatur di dalam grup!', quoted);
                    const sub = (args[0] || '').toLowerCase();
                    if (!['on', 'off', 'status'].includes(sub)) {
                        return reply(jid, `❌ Format: ${prefix}bacot on / ${prefix}bacot off / ${prefix}bacot status`, quoted);
                    }

                    if (sub === 'status') {
                        return reply(jid, `ℹ️ Mode bacot/nyinyir di grup ini sedang: *${isBacotOn(jid) ? 'ON 🔥' : 'OFF 🔇'}*`, quoted);
                    }

                    // hanya owner bot atau admin grup yang boleh ubah on/off
                    let boleh = isOwner(senderInfo);
                    if (!boleh) {
                        try {
                            const meta = await sock.groupMetadata(jid);
                            const p = meta.participants.find(pt => pt.id === sender || pt.id === senderInfo.participant);
                            boleh = !!p && (p.admin === 'admin' || p.admin === 'superadmin');
                        } catch {
                            // gagal ambil metadata grup -> anggap nggak boleh, aman secara default
                        }
                    }
                    if (!boleh) return reply(jid, '❌ Hanya admin grup atau owner bot yang boleh ubah setting ini!', quoted);

                    if (sub === 'off') {
                        if (!bacotOffGrp.includes(jid)) { bacotOffGrp.push(jid); writeDB('bacotoff.json', bacotOffGrp); }
                        await reply(jid, '🔇 Mode bacot/nyinyir di grup ini sudah di-*OFF*-kan. Bot bakal diem aja kalau di-reply.', quoted);
                    } else {
                        bacotOffGrp = bacotOffGrp.filter(g => g !== jid);
                        writeDB('bacotoff.json', bacotOffGrp);
                        await reply(jid, '🔥 Mode bacot/nyinyir di grup ini sudah di-*ON*-kan. Siap-siap diladenin kalau ngegas ke bot 😈', quoted);
                    }
                }
                ...
                // ═══════════════════════════════════════
                    ...
                    gameState.tebakLagu[jid] = {
                        ...
                        timeout: setTimeout(() => {
                            ...
                            sendTracked(jid, { text: `⏰ Waktu habis! Jawabannya: *${soal.jawaban}* - ${soal.artist}` });
                        }, 60000)
                    };
                ...
                // ═══════════════════════════════════════
                    ...
                    gameState.family100[jid] = {
                        ...
                        timeout: setTimeout(() => {
                            ...
                            if (!g) return;
                            sendTracked(jid, { text: `⏰ Waktu habis!\nSisa jawaban: ${g.sisa.join(', ')}` });
        ...
        for (const msg of messages) {
            ...
            // Auto adu bacot/nyinyir: balas kalau orang reply pesan BOT sendiri.
            // KHUSUS GRUP, dan cuma jalan kalau mode bacot grup itu ON.
            // Kalau pesannya ngandung kata kasar tertentu, bot nyari balasan
            // yang nyambung sama kata kasar itu (bukan asal balasan random).
            //
            // Deteksi "reply ke bot" PRIORITAS pakai stanzaId (ID pesan yang
            // beneran kita kirim sendiri, ada di sentMsgIds) karena ini akurat
            // 100%. Cek pakai JID participant cuma fallback, soalnya kadang
            // WhatsApp ngirim participant dalam bentuk @lid yang nggak match
            // sama JID asli bot meskipun itu beneran reply ke pesan bot
            // (ini yang kemarin bikin fitur nyinyirnya nggak nyala).
            const ctx = msg.message?.extendedTextMessage?.contextInfo;
            const matchByStanza = !!ctx?.stanzaId && sentMsgIds.has(ctx.stanzaId);
            const matchByJid = !!ctx?.participant && !!sock.user?.id &&
                jidNormalizedUser(ctx.participant) === jidNormalizedUser(sock.user.id);
            const isReplyToBot = matchByStanza || matchByJid;
            if (isReplyToBot && isGroup(jid) && isBacotOn(jid)) {
                await sendTracked(jid, { text: getBacotReply(body) }, { quoted: msg });
                continue;
            }
            ...
            if (gameState.tebakLagu[jid]) {
                ...
                if (body.toLowerCase().trim() === game.jawaban) {
                    ...
                    await sendTracked(jid, { text: `🎉 @${sender.split('@')[0]} BENAR!\n🎵 Jawabannya: *${game.jawaban}* - ${game.artist}` }, { quoted: msg });
                }
            }
            ...
            if (gameState.family100[jid]) {
                ...
                if (idx !== -1) {
                    ...
                    if (game.sisa.length === 0) {
                        clearTimeout(game.timeout);
                        delete gameState.family100[jid];
                        await sendTracked(jid, { text: `🎉 Semua jawaban ditemukan! Skor: ${game.score}` }, { quoted: msg });
                    } else {
                        await sendTracked(jid, { text: `✅ @${sender.split('@')[0]} benar! (+1)\nSisa: ${game.sisa.length} jawaban` }, { quoted: msg });
                    }
                }
            }
                ...
                if (!isNaN(num) && num >= 0 && num <= 8 && sender === game.currentTurn) {
                    ...
                    if (result === 1) {
                        ...
                        if (winner) {
                            ...
                            await sendTracked(jid, { text: `${board}\n\n🏆 @${winner.split('@')[0]} MENANG!` }, { quoted: msg });
